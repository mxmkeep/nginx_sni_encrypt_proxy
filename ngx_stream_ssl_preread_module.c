...
typedef struct {
    ngx_flag_t      enabled;
    ngx_flag_t      sni_encrypt;    //SNI encode:  - Matt.Ma 2023-10-12
    ngx_flag_t      sni_decrypt;    //SNI encode:  - Matt.Ma 2023-10-12
...
static ngx_int_t ngx_stream_ssl_preread_parse_record(
    ngx_stream_ssl_preread_ctx_t *ctx, u_char *pos, u_char *last, ngx_stream_session_t *s);
...
static ngx_command_t  ngx_stream_ssl_preread_commands[] = {
    ...
    { ngx_string("ssl_preread"),
      ...
      NULL },
    { ngx_string("ssl_sni_encrypt"),        //SNI encode:  - Matt.Ma 2023-10-12
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_FLAG,
      ngx_conf_set_flag_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_preread_srv_conf_t, sni_encrypt),
      NULL },
    { ngx_string("ssl_sni_decrypt"),        //SNI encode:  - Matt.Ma 2023-10-12
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_FLAG,
      ngx_conf_set_flag_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_preread_srv_conf_t, sni_decrypt),
      NULL },
    { ngx_string("key_base64"),
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|NGX_CONF_TAKE1,
      ngx_conf_set_str_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_ssl_preread_srv_conf_t, key_base64),
      NULL },
      ngx_null_command
};
...
ngx_stream_ssl_preread_handler(ngx_stream_session_t *s)
{
    ...
    while (last - p >= 5) {
        ...
        rc = ngx_stream_ssl_preread_parse_record(ctx, p, p + len, s);
...
//SNI encode:  - Matt.Ma 2023-10-12
#include "rijndael.h"
#if 0
static ngx_str_t g_encrypt_key = ngx_string("sni_encrypt_test");
static void simple_encrypt(u_char* str, uint32_t len) {

    uint i = 0;
    for (; i < len; i++) {
        str[i] ^= g_encrypt_key.data[i % g_encrypt_key.len];
    }
}
#endif
////////////////////////////////
...
ngx_stream_ssl_preread_parse_record(ngx_stream_ssl_preread_ctx_t *ctx,
    u_char *pos, u_char *last, ngx_stream_session_t *s)
{
    ...
    uint8_t read_sni = 0; //SNI encode:  - Matt.Ma 2023-10-12
    ngx_stream_ssl_preread_srv_conf_t  *sscf = ngx_stream_get_module_srv_conf(s, ngx_stream_ssl_preread_module);

    for ( ;; ) {
        n = ngx_min((size_t) (last - pos), size);

        //SNI encode:  - Matt.Ma 2023-10-12
        if(sscf->sni_decrypt && read_sni){
            //simple_encrypt(pos, n);
            u_char* out = ngx_pcalloc(ctx->pool, n);
            u_char* in = ngx_pcalloc(ctx->pool, n);
            ngx_memcpy(in, pos, n);
            size_t delen = rij_decrypt(in, n, &sscf->key_base64, out, ctx->pool);
            ngx_memmove(pos+delen, pos+n, s->connection->buffer->last - pos - n);
            ngx_memcpy(pos, out, delen);
            uint8_t reduce = n-delen;
            *(pos-1) -= reduce;        //server name length
            *(pos-4) -= reduce;        //server name llist
            *(pos-6) -= reduce;        //server name total  
            pos = pos + delen - n;
            last = last + delen - n;
            ctx->host.len = delen;
            read_sni = 0;
        ...
        //SNI encode:  - Matt.Ma 2023-10-12
        if(sscf->sni_encrypt && read_sni){
            //simple_encrypt(pos, n);
            ngx_uint_t pt_len = n + RIJNDAEL_BLOCKSIZE + 2;
            u_char* in = ngx_pcalloc(ctx->pool, pt_len);
            u_char* out = ngx_pcalloc(ctx->pool, pt_len+32);
            ngx_memcpy(in, pos, n);
            size_t enlen = rij_encrypt(in, n, &sscf->key_base64, out, ctx->pool);
            //ngx_memset(out,'e',enlen);
            size_t buff_remain = s->connection->buffer->end - s->connection->buffer->last;
            if(buff_remain > (enlen-n)) {
                ngx_memmove(pos+enlen, pos+n, s->connection->buffer->last - pos - n);
                ngx_memcpy(pos, out, enlen);
                uint8_t add = enlen-n;
                *(pos-1) += add;        //server name length
                *(pos-4) += add;        //server name llist
                *(pos-6) += add;        //server name total  
                // uint16_t exlen = add+*(pos-10);
                // *(pos-10) = exlen%256;   //ext length
                // *(pos-11) += exlen/256;
                pos = pos + enlen - n;
                last = last + enlen - n;
            } else {
        ...
        case sw_sni_host_head:
            ...
            state = sw_sni_host;
            dst = ctx->host.data;
            ctx->host.len = size; //SNI encode:  - Matt.Ma 2023-10-17

            read_sni = 1;   //SNI encode:  - Matt.Ma 2023-10-12

            break;

        case sw_sni_host:
            //ctx->host.len = (p[1] << 8) + p[2]; //SNI encode:  - Matt.Ma 2023-10-17
...
ngx_stream_ssl_preread_create_srv_conf(ngx_conf_t *cf)
{
    ...
    conf->sni_encrypt = NGX_CONF_UNSET;//SNI encode:  - Matt.Ma 2023-10-12
    conf->sni_decrypt = NGX_CONF_UNSET;//SNI encode:  - Matt.Ma 2023-10-12
...
ngx_stream_ssl_preread_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_value(conf->sni_encrypt, prev->sni_encrypt, 0);
    ngx_conf_merge_value(conf->sni_decrypt, prev->sni_decrypt, 0);
    ngx_conf_merge_str_value(conf->key_base64, prev->key_base64, "");
